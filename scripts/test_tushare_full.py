...
def test_tushare_data_service():
    """测试 Tushare 数据服务"""
    print("=" * 60)
    print("测试 Tushare 数据服务（第三方镜像）")
    print("=" * 60)
...
def test_get_stock_list(data_service):
    """测试获取股票列表"""
    print("\n" + "=" * 60)
    print("测试3：获取股票列表（上交所前20只）")
    print("=" * 60)

    try:
        df = data_service.adapter.get_stock_list(exchange='SSE')
...
def run_all_tests():
    """运行所有测试"""
    print("\n" + "🚀 开始完整测试 Tushare 数据源（第三方镜像）" + "\n")

    # 测试连接
    data_service = test_tushare_data_service()
    if not data_service:
        print("\n❌ 数据服务初始化失败，无法继续测试")
        return 1
